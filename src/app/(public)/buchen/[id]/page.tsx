        ...
        return (
            <div className="min-h-screen bg-white flex items-center justify-center">
                <div className="text-center">
                    <h1 className="font-['Inter'] font-semibold text-[24px] text-[#1a202c] mb-4">Produkt nicht gefunden</h1>
                    <Link href="/produkte"><Button variant="secondary">Zurück zu den Produkten</Button></Link>
                </div>
            </div>
        );
    }
                ...
                <div className="max-w-[800px] mx-auto px-4 sm:px-6 lg:px-8 py-12">
                    <div className="bg-white rounded-[8px] border border-[#cbd5e1] p-8 md:p-12 text-center">
                        ...
                        <p className="font-['Inter'] text-[16px] text-[#4a5568] leading-[25.6px] mb-8">
                            Vielen Dank für Ihre Buchungsanfrage für <strong>{product.title}</strong>. Wir haben Ihre Anfrage erhalten und melden uns schnellstmöglich bei Ihnen.
                        </p>
            ...
            <div className="max-w-[1000px] mx-auto px-4 sm:px-6 lg:px-8 py-12">
                <div className="mb-8">
                    <nav className="flex items-center gap-2 text-[14px]">
                        ...
                        <span className="text-[#cbd5e1]">/</span>
                        <Link href={`/produkt/${product.id}`} className="font-['Inter'] text-[#64748b] hover:text-[#1a3a52]">{product.title}</Link>
                        <span className="text-[#cbd5e1]">/</span>
                ...
                <div className="bg-[#e2e8f0] rounded-[8px] p-6 mb-8 flex flex-col sm:flex-row gap-6">
                    <img src={product.imageUrl} alt={product.title} className="w-full sm:w-[150px] h-[100px] object-cover rounded-[8px]" />
                    <div className="flex-1">
                        <h2 className="font-['Inter'] font-medium text-[18px] text-[#1a202c] mb-2">{product.title}</h2>
                        <p className="font-['Inter'] font-semibold text-[16px] text-[#4a5568]">{product.price}</p>
                    </div>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 pt-4">
                        ...
                        <Link href={`/produkt/${product.id}`} className="flex-1">
